...
(deploy step off), zips it as `FFFCheat-X.Y.zip` (the `.asi` plus the default `FFFCheat.ini`), publishes a GitHub release named
"FFFCheat X.Y.Z" with that changelog entry as its notes and the zip and `.asi`
...
`bin\<Configuration>\FFFCheat.ini` is generated on every Debug/Release build
